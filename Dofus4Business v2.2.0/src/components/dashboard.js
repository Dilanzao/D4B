...
export function renderDashboard(state) {
  ...
  const filterPanel=`<article class="card section dashboard-filter-card"><div class="section-head compact-head"><div><span class="eyebrow">${escapeHtml(t(state,'dashboard.filters'))}</span><h2>${escapeHtml(t(state,'dashboard.title'))}</h2><p>${escapeHtml(t(state,'dashboard.description'))}</p></div></div><div class="dashboard-filters">
    ...
    <label class="field"><span>${escapeHtml(t(state,'dashboard.creatureTypes'))}</span><select class="select" data-dashboard-filter="petMetric">${['quantity','revenue','profit'].map(v=>`<option value="${v}" ${f.petMetric===v?'selected':''}>${escapeHtml(t(state,`dashboard.${v}`))}</option>`).join('')}</select></label>
    <label class="field"><span>${escapeHtml(t(state,'dashboard.channels'))}</span><select class="select" data-dashboard-filter="channelMetric">${['quantity','revenue','profit','margin'].map(v=>`<option value="${v}" ${f.channelMetric===v?'selected':''}>${escapeHtml(t(state,`dashboard.${v}`))}</option>`).join('')}</select></label>
    <label class="field"><span>${escapeHtml(t(state,'dashboard.methods'))}</span><select class="select" data-dashboard-filter="methodMetric">${['averageCost','averageProfit','averageMargin','quantity'].map(v=>`<option value="${v}" ${f.methodMetric===v?'selected':''}>${escapeHtml(t(state,`dashboard.${v}`))}</option>`).join('')}</select></label>
  </div></article>`;
