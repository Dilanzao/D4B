...
function comparison(state, line) {
  ...
  const craftCost = line.subRecipe.map(child => ingredientCost(child,line.remaining,state.craftInventory)).reduce((sum, child) => sum + child.cost, 0);
...
function recipeLine(state, line) {
  ...
  return `<article class="recipe-planner-line" data-ingredient-id="${escapeHtml(line.id)}">
    ...
    <label class="field compact-field"><span>${escapeHtml(canCraft?t(state,'v303.readyMarketPrice'):t(state,'v3.crafts.unitPrice'))}</span><input class="input" inputmode="numeric" data-craft-ingredient-field="unitMarketPrice" data-id="${escapeHtml(line.id)}" value="${line.unitMarketPrice||''}"></label>
...
export function renderCraftRecipePlanner(state, modal) {
  ...
  const rows = (current.subRecipe || []).map(child => ingredientCost(child,multiplier,state.craftInventory));
  const body = `<div class="recipe-planner">
